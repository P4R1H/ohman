...
namespace Ohman {
    ...
    public sealed class KeyboardView : FrameworkElement {
        ...
        protected override void OnRender(DrawingContext dc) {
            double pad = Interactive ? 10 : 3, gap = Interactive ? 1.6 : 0.6;
            double scale = (ActualWidth - 2 * pad) / unitsW;
            double h = unitsH * scale + 2 * pad;
            dc.DrawRoundedRectangle(Ui.Brush("#141820"), new Pen(Ui.Brush("#1F232C"), 1), new Rect(0.5, 0.5, ActualWidth - 1, h - 1), Interactive ? 10 : 4, Interactive ? 10 : 4);
            double radius = Interactive ? 3.5 : 1.2;
            foreach (var k in Keys) {
                var r = KeyRect(k, scale, pad, gap);
                Color fill;
                if (Off || ZoneColors.Length == 0) fill = Color.FromRgb(0x2A, 0x2F, 0x3A);
                else { var c = ZoneColors[Math.Min(ZoneColors.Length - 1, PerKey ? Math.Min(k.Index, ZoneColors.Length - 1) : k.Zone)]; fill = Color.FromRgb(c.R, c.G, c.B); }
                bool sel = Interactive && Selected.Contains(PerKey ? k.Index : k.Zone);
                Pen pen = sel ? new Pen(Brushes.White, 1.5) : (Interactive && k.Index == hover ? new Pen(Ui.Brush("#A4ADBF"), 1) : null);
                dc.DrawRoundedRectangle(new SolidColorBrush(fill), pen, r, radius, radius);
                if (Interactive && k.Label.Length > 0 && r.Width > 14) {
                    var ft = new FormattedText(k.Label, CultureInfo.InvariantCulture, FlowDirection.LeftToRight, Face, k.Label.Length > 2 ? 8.5 : 9.5, Off ? Ui.Brush("#6B7385") : Ui.Brush("#0E1014"), 1.0);
                    dc.DrawText(ft, new Point(r.X + 4, r.Y + 3));
                }
            }
        }
